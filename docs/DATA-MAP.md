...
| 어디 | 무엇 |
|---|---|
| `contracts.annualSalary` / `startDate` | **원천** — 사람이 입력하는 곳 |
| `employees.currentSalary` / `lastContractDate` | **파생** — 최신 계약에서 계산된 표시용 값 |

`domain/hr.js` 의 **`syncEmployeeContractFields(employeeId)`** 가 계약에서 두 필드를
다시 계산합니다. **계약을 추가·삭제한 뒤에는 반드시 부르세요.** 지금 부르는 곳은
직원 상세 서랍(계약 등록/갱신/삭제), 신규 직원 등록, 엑셀 업로드입니다.

직원 수정 폼에서 이 두 필드는 **읽기 전용**입니다. 편집 가능하게 되돌리지 마세요 —
예전에 그래서 "계약은 있는데 직원 연봉이 0", "직원 연봉은 있는데 계약이 없음" 이
동시에 생겼습니다.
...
**계약 종료일, 없으면 시작일 + 12개월**. 인력 마스터 목록과 계약 관리 대기 리스트가
같은 값을 보여야 하므로 각자 계산하지 마세요.

계약 관리 대기 리스트(`contractRenewalDueList()`)도 직원 필드가 아니라 **계약을 직접**
봅니다. 예전에는 `lastContractDate` 를 기준으로 삼았는데, 그 값을 채우는 경로가
"계약 입력"(갱신) 하나뿐이라 "계약 등록" 으로 넣은 계약이 이 화면에 나타나지 않았습니다.
