...
- **연봉은 계약 한 곳에서만 관리됩니다** — 계약을 등록하면 직원의 현재 연봉과
  최종 계약일이 자동으로 따라갑니다. 아래 "화면 동작 규칙" 참고.
- **수습은 재직상태만 고르면 자동으로 잡힙니다** — 인력 마스터에서 "수습" 으로
  등록하면 수습 관리 대기 리스트에 바로 나타납니다.
- **기본 정보는 그 자리에서 수정합니다** — 모달로 넘어가지 않고 각 항목 박스가
  입력칸으로 바뀌며, "완료" 를 누르면 다시 읽기 모드가 됩니다.
...
src/
  ...
  domain/
    hr.js                  ★ 인사 규칙 — 계약→직원 연봉 동기화, 수습 정보 채우기,
                           재계약 대상 판정, 지원금 상태 동기화
    employee-helpers.js    최신 계약 고르기, 재계약 예정일 계산, 최근 수습평가일
...
**★ 표시가 "어떤 데이터를 바라보는지 / 어떤 규칙이 걸려 있는지"의 핵심입니다.**
기능을 붙이기 전에 [`docs/DATA-MAP.md`](docs/DATA-MAP.md) 와 아래
["화면 동작 규칙"](#화면-동작-규칙) 을 먼저 보세요.
...
같은 사실을 두 곳에서 고칠 수 있으면 반드시 갈라집니다. 아래는 그런 곳을
하나로 모아 둔 규칙이라, 화면을 고칠 때 먼저 확인하세요.

### 연봉 · 최종 계약일 — 계약이 원천

```
계약(contracts)  ──최신 1건에서 계산──>  직원.현재연봉 · 직원.최종계약일
    ↑ 여기서만 입력                          ↑ 읽기 전용 (표시용)
```

- `domain/hr.js` 의 **`syncEmployeeContractFields(employeeId)`** 가 재계산합니다.
  계약을 **등록·갱신·삭제한 뒤 반드시** 부르세요. 지금 부르는 곳은 직원 상세 서랍,
  신규 직원 등록, 엑셀 업로드입니다.
- 직원 **수정** 화면에서 연봉·최종 계약일은 **읽기 전용**입니다. 편집 가능하게
  되돌리지 마세요 — 예전에 그래서 "계약은 있는데 직원 연봉이 0", "직원 연봉은
  있는데 계약이 없음" 이 동시에 생겼습니다.
- 신규 직원 등록에서 연봉을 넣으면 **첫 계약이 자동 생성**됩니다
  (`contractTypeFor()` 가 고용형태에 맞는 계약 구분을 고릅니다).
- 계약 등록 모달은 직원을 고르면 **현재 연봉과 계약 구분을 채웁니다.** 연봉이
  비어 있으면 저장되지 않습니다 — 0 으로 저장되면 직원 연봉까지 0 이 됩니다.
- 최신 계약은 **시작일이 같으면 나중에 등록한 것**입니다
  (`pickLatestContract()`). 같은 날 여러 번 등록하는 일이 흔합니다.

### 재계약 예정일

`employee-helpers.js` 의 **`renewalDateOf(contract)`** 하나만 씁니다 —
**계약 종료일, 없으면 시작일 + 12개월**. 인력 마스터 목록과 계약 관리가 같은
값을 보여야 하므로 각자 계산하지 마세요.

계약 관리 화면의 세 목록이 뜻하는 것:

| 목록 | 무엇 |
|---|---|
| 대기 리스트 | 재계약 예정일이 알림 기준일 이내이거나 지난 직원 |
| 전체 계약 | 등록된 모든 계약 (최신순). 각 직원의 현재 계약에 "현재" 표시 |
| 완료 리스트 | 상태가 "완료" 인 계약 |

> 방금 시작한 계약이 대기 리스트에 없는 것은 정상입니다. 만료가 임박하지
> 않았기 때문이고, "전체 계약" 에서 확인할 수 있습니다.

계약 상태(대기/지연/완료)는 **표 안 드롭다운에서 바로** 바꿉니다. 표시값은
저장된 `status` 그대로입니다 — `contractEffectiveStatus()` 는 종료일이 지나면
"지연" 으로 보이게 하는 계산이라 선택값으로 쓰면 안 됩니다.

### 수습

수습 관리는 `probation.startDate` 가 있어야 대상으로 잡습니다. `domain/hr.js` 의
**`ensureProbationInfo(data)`** 가 재직상태 "수습" 인데 시작일이 없으면
**업무시작일 → 입사일 → 오늘** 순으로 골라 시작일로 잡고
`PROBATION_MONTHS`(3개월) 뒤를 종료일로 둡니다. 신규 등록과 정보 수정 양쪽에서
부릅니다. 이미 시작한 수습은 건드리지 않습니다.

### 날짜 계산

`addMonths()` 는 **UTC 로 계산**합니다. 로컬 파싱 + `toISOString()` 조합은 KST 에서
결과가 늘 하루 앞당겨집니다. 말일은 다음 달 말일로 맞춥니다(1/31 + 1개월 = 2/28).

### 화면 접근 제한

`PMO_RESTRICTED_ROUTES` 를 **세 곳**이 봅니다 — 메뉴 숨김, `setRoute()`,
그리고 **`renderRoute()`**. 마지막 것을 빼면 안 됩니다. `route` 는 모듈 변수라
로그아웃해도 남고 로그인 직후 경로는 `setRoute()` 를 거치지 않아서, 관리자가
시스템 설정을 보던 상태에서 PMO 로 바꿔 로그인하면 그 화면이 그대로 남았습니다.
