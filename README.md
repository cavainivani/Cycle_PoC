...
- **동시 편집 시 나중에 저장한 쪽이 이깁니다.** 두 사람이 같은 직원을 동시에 열어
  저장하면 앞사람의 수정이 조용히 덮입니다. 어댑터가 "레코드 전체" 를 보내는
  구조라, 고치려면 프론트엔드 계약까지 손대야 합니다.
...
2. **동시 편집 시 나중에 저장한 쪽이 이깁니다.**
   두 사람이 같은 직원을 동시에 열어 저장하면 앞사람의 수정이 조용히 덮입니다.
   어댑터 계약이 "레코드 전체" 를 보내는 구조라 서버가 무엇이 바뀌었는지 알 수 없습니다.
   고치려면 프론트엔드 계약까지 손대야 합니다(버전 필드 + 충돌 시 409 등).
